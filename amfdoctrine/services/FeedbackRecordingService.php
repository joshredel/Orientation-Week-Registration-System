...
class FeedbackRecordingService {
	public function getFeedbackRecordings() {
		// bootstrap to doctrine
		require(__DIR__ . '/../../amfdoctrine/bootstrapper.php');
		
		// create the select query and execute
		$dql = "SELECT u FROM org\\fos\FeedbackRecording u ORDER BY u.id ASC";
	...
	public function getFeedbackRecording($recordingId) {
		// bootstrap to doctrine
		require(__DIR__ . '/../../amfdoctrine/bootstrapper.php');
		
		// create the select query and execute
		$dql = "SELECT u FROM org\\fos\FeedbackRecording u WHERE u.id=$recordingId";
		$query = $entityManager->createQuery($dql);
		$recordings = $query->getResult();
	...
	public function saveFeedbackRecording($recording) {
	...
	public function deleteFeedbackRecording($recording) {
		// stop if there was nothing passed
		if($recording == null) {
			return;
		}
	...
	public function deleteFeedbackRecordingById($recordingId) {
		// stop if there was nothing passed
		if($recordingId == null) {
			return;
		}
