...
$selectedFaculty = $participant->faculty;
$selectedLivingStyle = $participant->livingStyle;
$selectedOrigin = $participant->placeOfOrigin;
...
<html lang="en"><head>
	<meta charset="utf-8">
	<title>myWeek | Modify Registration</title>
...
<body class="sign-up">
    ...
    <script>
		...
		*/
		...
		// an array of unregisterable events
		var unregisterableEventIds = new Array();
		...
		function removeEventFromTicket(eventId) {
			// check to see if we are allowed to remove this
			for(var i = 0; i < unregisterableEventIds.length; i++) {
				if(unregisterableEventIds[i] == eventId) {
					// this event has been paid for, so it cannot be removed
					alert("This event has already been paid for, so you cannot unregister for it from here.  Please contact the event organizer to receive a refund before unregistering.");
					return;
				}
			}
		...
		function registerSessionEvents(rawSessionData) {
			if(rawSessionData != null && rawSessionData != "") {
		...
		// make this event ID unregisterable
		function markEventUnregisterable(eventId) {
			unregisterableEventIds.push(eventId);
		}
    	...
    	<div class="container">
        	<div class="row-fluid">
            	<div class="well span12 sign-up">
                	<h2 class="text-center">myWeek<br />Modify Registration</h2>
                    
                    <a href="index.php?passkey=<?= $_GET['passkey'] ?>" class="btn-main-small"><i class="icon-chevron-left"></i> Back to myWeek</a><br /><br />
                    	...
                    	<div class="span4">
                        	<div id="float-menu">
                                <div class="boxed">
                                    ...
                                    <br />
                                    <form id="eventRegistrationForm" method="POST" action="/actions/myweek/processChangeRegistration.php" onSubmit="saving = true">
                                    	<input type="hidden" id="registeredEvents" name="registeredEvents">
                                        <input type="hidden" id="passkey" name="passkey" value="<?= $_GET['passkey'] ?>">
                                        <input type="hidden" name="save" value="changeRegistrationDone">
                                    	<button class="btn btn-main-small">Save Registration <i class="icon-chevron-right"></i></button>
                                    </form>
                                    ...
                                    <div id="orientationplus" class="accordion-body collapse">
                                        <div class="accordion-inner">
                                            ...
                                            try {
												...
												echo("<p>We are working on some of the events in this category.  Please try again in 30 minutes to register for more events from this category.</p>");
												//echo("<script>window.location = '/myweek/error.php?passkey=" . $_GET['passkey'] . "&message=" . urlencode("We are currently working to resolve an error with registrations.  Rest assured that you will be able to register for more events in the near future.  Please try again in 30 minutes.") . "'</script>");
											}
                                            ?>
                                        </div>
                                    </div>
    ...
    <script>
		...
		<?
		// load the events we are already registered for
		$registeredEvents = $participant->rawRegistrationData;
		echo("registerSessionEvents(\"" . $registeredEvents . "\");");
		
		// create a list of events that cannot be unregistered (events with paid payments)
		$unregisterableEventIds = array();
		foreach($participant->payments as $payment) {
			if($payment->hasPaid) {
				echo("markEventUnregisterable(" . $payment->event->id . ");");
			}
		}
		?>
		...
		$(window).on('beforeunload', function() {
			if(!saving) {
				if(formIsDirty) {
					return "You haven't saved changes you have made on this page.  Please use the 'SAVE REGISTRATION' button to save your changes and continue.";
				}
			}
		});
