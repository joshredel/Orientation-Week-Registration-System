...
$_SESSION['lastParticipantLocation'] = "register.php";
...
<head>
	...
	function toggleOptionPrice() {
		...
		if(checkbox.checked) {
			// show the full option price in the price field
			document.getElementById("rate").innerHTML = "$<?= $totalEventCost ?>";
			document.getElementById("checkInTotalRate").value = "$<?= $totalEventCost ?>";
		} else {
			// show the base price in the price field
			document.getElementById("rate").innerHTML = "$<?= $totalEventBaseCost ?>";
			document.getElementById("checkInTotalRate").value = "$<?= $totalEventBaseCost ?>";
		}
	}
	...
	function checkin(firstName, lastName, amount, studentId, passkey, method, paid, merch, bracelet, braceletNo, eventAge, option, phone, customAnswers) {
		// Clear form (Just in case)
		clearCheckin();
		
		// set form to current State
		document.getElementById("rate").innerHTML = amount;
		document.getElementById("checkInTotalRate").value = amount;
	...
	function clearCheckin() {
		...
		document.getElementById("checkInTotalRate").value = "";
...
<body <?= ($currentMode == "waiting" ? "onLoad='document.idform.studentId.focus();'" : "") ?>>
	<div id='container'>
    	<div id='header'>
    	...
    	<section id='content'>
			...
			if(!$currentRole->hasPermission(org\fos\Role::$CHECK_IN_PARTICIPANTS)) {
				...
				if($currentMode == "waiting") {
					...
					?>
            		<article>
                    	<header><h1>Register - Check for Existing ID</h1></header>
                        <p>
                        	Please enter the participant's student ID.  It will be checked for an existing student.
                        </p>
                        <form method="post" name="idform" onsubmit='return checkIDForm(this)'>
                        	<label for='firstName'><b>Student ID:</b></label>
                   			<input type='text' name='studentId' maxlength='9' size='10' /><br />
                	...
                	<article id="checkinBox">
                        ...
                        <!-- start fresh -->
                        <p>
                        	A participant with student ID <?= $studentId ?> does not yet exist.<br /><br />
							Please direct them to the Information Kiosk so that they can perform initial registration.
                        </p>
                        
                        <button class="button" onClick="window.location = 'register.php';">Back to Cross Registration</button>
                        <? } else {
							if(inDoctrineArray($currentEvent, $participant->events)) {
								?>
                                <p>
                                	A participant with student ID <?= $participant->studentId . ' (' . getDisplayName($participant) . ' ' . $participant->lastName . ')'?> is already 
                                    registered for <strong><?= $currentEvent->eventName ?></strong>.
                                    <br /><br />
                                    <button class="button" onClick="window.location = 'register.php';">Back to Cross Registration</button>
                                </p>
                                <?
                                ...
                                <!-- ask to import -->
                                <p>
                                    A participant with student ID <?= $participant->studentId . ' (' . getDisplayName($participant) . ' ' . $participant->lastName . ')'?> already exists.
                                </p>
                                <p>
                                    Fill out the form below to cross register them and check them in to <strong><?= $currentEvent->eventName ?></strong>.
                                </p>
                                <?
								$warningClass = "";
								if($percentageComplete > 98) {
									$warningClass = "red-text";
								}
								?>
                                <p class="<?= $warningClass ?>">
                                	Your event is currently at <?= $percentageComplete ?>% capacity (<?= $numberRegistered ?> of <?= $currentEvent->participantCap ?>).  Cross registration will allow you to exceed your capacity of <?= $currentEvent->participantCap ?> people.  Watch out!
                                </p>
                                    ...
                                    <table class="checkInTable">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                ...
                                                <input type="button" class="button" style="display: inline-block" onClick="window.location = 'register.php'" value="Cancel" />
                                            </td>
                                        </tr>
                                    </table>
                                    
                                    <input id="checkInPasskey" type="hidden" name="passkey" value="" />
                                    <input id="checkInRegister" type="hidden" name="register" value="yes" />
                                    <input id="checkInTotalRate" type="hidden" name="totalRate" />
                                    <!--
                                    <input class="redbutton" style="display: inline-block" type="submit" value="&nbsp;&nbsp;&nbsp;Check-In&nbsp;&nbsp;&nbsp;" />
                                    <input type="button" class="button" style="display: inline-block" onClick="window.location = 'register.php';" value="Cancel &amp; return" />
                                    -->
                                </form>
                                <?
							}
    ...
    <script>
		$(document).ready(function() {
			<?
			if($currentMode != "waiting" && $participant != null && !inDoctrineArray($currentEvent, $participant->events)) {
				echo("checkin(\"" . getDisplayName($participant) . "\", \"{$participant->lastName}\", \"$" . $totalPaid . "\", \"{$participant->studentId}\", \"{$participant->registrationPassword}\", \"$paymentMethod\", $paid, $merch, $bracelet, \"$braceletNumber\", \"$eventAge\", $selectedOption, \"$phone\", \"$customAnswers\");");
			}
			?>
      	});
	</script>
