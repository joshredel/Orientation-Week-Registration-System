...
if($reportType == "full") {
	...
	$header = array("Participant ID", "Last Name", "First Name", "Preferred Name", "Preferred Pronoun", "Student ID", "Email", "Faculty", "Date of Birth", "Shirt Size", "Dietary Restrictions", "Allergies", "Physical Needs", "Place of Origin", "Entering Year", "Registration Date", "Living Style", "Group Number");
	...
	foreach($participants as $participant) {
		...
		// prepare the group number
		if($participant->groupNumber != null && $participant->groupNumber != "") {
			$groupInfo = split("::", $participant->groupNumber);
			$participantData[] = $groupInfo[1];
		} else {
			$participantData[] = "";
		}
	...
	/////////
	// print a header
	$outputData[] = array("Participant ID", "Last Name", "Preferred Name", "Student ID", "Email", "Payment Method", "Payment Amount", "Payment Status", "Payment Date");
	...
	foreach($participants as $participant) {
		...
		if($currentEvent != null) {
			...
			if($eventPayment != null) {
				...
				if($eventPayment->paymentDate != null) {
					$payDate = formatIsoDateTime($eventPayment->paymentDate);
				} else {
					$payDate = "";
				}
			} else {
		...
		// print the info
		$outputData[] = array($participant->id, toCellPrint($participant->lastName), getDisplayName($participant), $participant->studentId, $participant->email, $method, $rate, $paymentStatus, $payDate);
	}
