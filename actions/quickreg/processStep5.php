...
if(!isset($_POST['save']) || $_POST['save'] != 'step5done') { 
...
if(!isset($_SESSION['step4Complete']) || $_SESSION['step3Complete'] != true) {
...
$bodyparams = array (	
	...
	"currencyCode" => "CAD",
	"requestEnvelope.errorLanguage" => "en_US",
	"cancelUrl" => "http://orientation.ssmu.mcgill.ca/registration/complete.php?status=cancelled&passkey=" . $participant->registrationPassword,
	"returnUrl" => "http://orientation.ssmu.mcgill.ca/registration/complete.php?status=completed&passkey=" . $participant->registrationPassword,
...
if(count($payPalPayments) > 0) {
	try {
		...
		if ($response === false) {
			...
			$errorMessage = "There was an error communicating with PayPal (stream not responding). You have been registered, but your payments have not been completed.  <a href=\'step5.php\'>Try again.</a>";
			redirect("/registration/complete.php?status=error&passkey=" . $participant->registrationPassword . "&error=" . urlencode($errorMessage));
		 }
		...
		if($kArray["responseEnvelope.ack"] == "Success") {
			...
			$errorMessage .= "There was an error communicating with PayPal. You have been registered, but your payments have not been completed.  <a href=step5.php>Try again.</a>";
			redirect("/registration/complete.php?status=error&passkey=" . $participant->registrationPassword . "&error=" . urlencode($errorMessage));
			//mail($developerEmail, "PAYMENTS", $errorMessage);
		}
	} catch(Exception $e) {
		echo "Message: ||" .$e->getMessage()."||";
		redirect("/registration/complete.php?status=error&passkey=" . $participant->registrationPassword . "&error=Exception: " . urlencode($e->getMessage));
	}
} else {
	...
	// all done!
	redirect("/registration/complete.php?status=completed&passkey=" . $participant->registrationPassword);
	exit();	
}
...
// all done!
redirect("/quickreg/complete.php?status=completed&passkey=" . $participant->registrationPassword);
exit();	
?>