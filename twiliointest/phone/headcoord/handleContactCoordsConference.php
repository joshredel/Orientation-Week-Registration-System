	...
	foreach($role->users as $user) {
		...
		if(($user->classification == org\fos\User::COORDINATOR || $user->classification == org\fos\User::HEAD_COORDINATOR) && $user->id != $caller->id) {
			if($user->phoneNumber != null && $user->phoneNumber != "") {
				...
				try {
					// make the call
					$call = $client->account->calls->create("+15149000125", $user->phoneNumber, "http://orientation.ssmu.mcgill.ca/twiliointest/phone/headcoord/handleConferenceCallCoords.php?from=" . urlencode($caller->displayName) . "&conference=" . $conferenceName . "&url=" . urlencode($recordingUrl), array("IfMachine" => "Hangup"));
					$successCount++;
				} catch (Exception $e) {
					$errors[] = $e;
					$errorCount++;
