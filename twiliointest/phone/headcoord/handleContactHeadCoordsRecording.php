...
foreach($users as $user) {
	...
	if($user->phoneNumber != $caller->phoneNumber) {
		if($user->phoneNumber != null && $user->phoneNumber != "") {
			...
			try {
				// make the call
				$call = $client->account->calls->create(nextNumber(), $user->phoneNumber, "http://orientation.ssmu.mcgill.ca/twiliointest/phone/headcoord/handleRecordingCall.php?from=" . urlencode($caller->displayName) . "&url=" . urlencode($recordingUrl), array("IfMachine" => "Continue"));
				$successCount++;
			} catch (Exception $e) {
				$errors[] = $e;
				$errorCount++;
...
try {
	...
	$message = $client->account->sms_messages->create($numberFrom, $caller->phoneNumber, "Your mass voice message to all head coords was sent successfully to $successCount people.", array("StatusCallback" => "http://orientation.ssmu.mcgill.ca/twiliointest/sms/textback.php"));
	$successCount++;
} catch (Exception $e) {
	$errors[] = $e;
	$errorCount++;
