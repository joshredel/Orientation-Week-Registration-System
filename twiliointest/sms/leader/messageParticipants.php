...
$message = "Message from leader $fromName: $original (message powered by http://twil.io)";
...
?>
<Response>
	<Sms>Your message is being sent to all members of your group.  You will receive confirmation when complete.</Sms>
</Response>
<?

// call all members of their group
$errors = array();
$errorCount = 0;
$successCount = 0;
$triedCount = 0;
	...
	if($participant->phoneNumber != null && $participant->phoneNumber != "") {
		...
		try {
			...
			foreach($messagesToSend as $messageToSend) {
				$message = $client->account->sms_messages->create($numberFrom, $participant->phoneNumber, $messageToSend, array("StatusCallback" => "http://orientation.ssmu.mcgill.ca/twiliointest/sms/textback.php"));
				$successCount++;
			}
			break;
		} catch (Exception $e) {
...
try {
	...
	$message = $client->account->sms_messages->create($numberFrom, $caller->phoneNumber, "Your mass text to all members of your group was sent successfully to $successCount people.", array("StatusCallback" => "http://orientation.ssmu.mcgill.ca/twiliointest/sms/textback.php"));
	$successCount++;
} catch (Exception $e) {
	$errors[] = $e;
	$errorCount++;
