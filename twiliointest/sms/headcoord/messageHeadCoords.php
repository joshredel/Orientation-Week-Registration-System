...
?>
<Response>
	<Sms>Your message is being sent to all head coords.  You will receive confirmation when complete.</Sms>
</Response>
<?
		...
		if($user->phoneNumber != null && $user->phoneNumber != "") {
			...
			try {
				...
				foreach($messagesToSend as $messageToSend) {
					$message = $client->account->sms_messages->create($numberFrom, $user->phoneNumber, $messageToSend, array("StatusCallback" => "http://orientation.ssmu.mcgill.ca/twiliointest/sms/textback.php"));
					$successCount++;
				}
			} catch (Exception $e) {
				$errors[] = $e;
...
try {
	...
	$message = $client->account->sms_messages->create($numberFrom, $caller->phoneNumber, "Your mass text to all head coords was sent successfully to $successCount people.", array("StatusCallback" => "http://orientation.ssmu.mcgill.ca/twiliointest/sms/textback.php"));
	$successCount++;
} catch (Exception $e) {
	$errors[] = $e;
	$errorCount++;
...
// send debugger email
mail("joshredel@mac.com", "Twilio: Head Coord to Head Coords Texting Info", "Tried: $triedCount\nSuccesses: $successCount\nFailuers: $errorCount\nEvent name: " . $currentEvent->eventName . "\nThe following errors occured:\n " . implode("\n", $errors));
?>