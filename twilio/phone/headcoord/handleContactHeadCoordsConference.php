...
foreach($users as $user) {
	...
	if($user->phoneNumber != $caller->phoneNumber) {
		if($user->phoneNumber != null && $user->phoneNumber != "") {
			...
			try {
				// make the call
				$call = $client->account->calls->create("+15149000125", $user->phoneNumber, "http://orientation.ssmu.mcgill.ca/twilio/phone/headcoord/handleConferenceCallHeadCoords.php?from=" . urlencode($caller->displayName) . "&conference=" . $conferenceName . "&url=" . urlencode($recordingUrl), array("IfMachine" => "Hangup", "StatusCallback" => "http://orientation.ssmu.mcgill.ca/twilio/phone/callback.php"));
				$successCount++;
			} catch (Exception $e) {
				$errors[] = $e;
				$errorCount++;
