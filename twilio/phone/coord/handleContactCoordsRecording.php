	...
	foreach($role->users as $user) {
		...
		if(($user->classification == org\fos\User::COORDINATOR || $user->classification == org\fos\User::HEAD_COORDINATOR) && $user->id != $caller->id) {
			if($user->phoneNumber != null && $user->phoneNumber != "") {
				...
				$triedCount++;
				try {
					$call = $client->account->calls->create(nextNumber(), $user->phoneNumber, "http://orientation.ssmu.mcgill.ca/twilio/phone/coord/handleRecordingCall.php?from=" . urlencode($caller->displayName) . "&url=" . urlencode($recordingUrl), array("IfMachine" => "Continue"));
					$successCount++;
				} catch (Exception $e) {
					$errors[] = $e;
					$errorCount++;
...
try {
	...
	$message = $client->account->sms_messages->create($numberFrom, $caller->phoneNumber, "Your mass voice message to all coords was sent successfully to $successCount people.", array("StatusCallback" => "http://orientation.ssmu.mcgill.ca/twilio/sms/textback.php"));
	$successCount++;
} catch (Exception $e) {
	$errors[] = $e;
	$errorCount++;
